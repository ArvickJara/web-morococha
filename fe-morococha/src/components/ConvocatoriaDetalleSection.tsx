...
interface Convocatoria {
  ...
  bases: Archivo[] | null;
  resultado_curricular: Archivo[] | null;
  resultado_entrevista: Archivo[] | null;
  resultado_final: Archivo[] | null;
  comunicados: Archivo[] | null;
...
const ConvocatoriaDetalleSection = ({ documentId }: Props) => {
  ...
  useEffect(() => {
    ...
    api
      .get<{ data: Convocatoria[] }>(
        `/convocatorias?filters[tipos_convocatoria][documentId][$eq]=${cleanId}&populate=*`
      )
      .then((res) => {
  ...
  const renderArchivos = (archivos: Archivo[] | null, emptyText: string) => {
      ...
      <div className="flex flex-wrap gap-2">
        {archivos.map((archivo) => (
          <button
            key={archivo.id}
            onClick={() =>
              window.open(`${ASSETS_URL}${archivo.url}`, "_blank")
            }
              ...
              <tbody>
                {convocatorias.map((convocatoria) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      {renderArchivos(convocatoria.resultado_curricular, "resultado curricular")}
                    </td>
                    <td className="px-4 py-3">
                      {renderArchivos(convocatoria.resultado_entrevista, "resultado entrevista")}
                    </td>
                    <td className="px-4 py-3">
                      {renderArchivos(convocatoria.resultado_final, "resultado final")}
                    </td>
                    <td className="px-4 py-3">
                      {renderArchivos(convocatoria.comunicados, "comunicados")}
                    </td>
                  </tr>
                ))}
              </tbody>
